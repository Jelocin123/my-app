...
import styles from "../../styles/ps3.module.scss";
...
const ps3 = () => {
  ...
  const fetchPs3Games = async () => {
    try {
      let orderBy;
  
      // Check if "Sort by Price" is selected and not in default sorting order
      if (sortBy === "2" && sortOrder !== "1") {
        orderBy = sortOrder === "3" ? { column: 'price', ascending: true } : { column: 'price', ascending: false };
      } else {
        orderBy = { column: 'id', ascending: true }; // Default order by 'id'
      }
      ...
      console.error('Error fetching PS4 games:', error.message);
    }
  };
    ...
    <div id="ps3_games">
      <div className="container" id={styles.cus_container}>
          ...
          <section className='col-6'>
            <div className='row'>
              <section className='col-xxl-2 col-xl-3 col-4 ms-1'>
                <p className='m-0' id={styles.PS5}><i className="bi bi-playstation"></i> PS3</p>
              </section>
            </div>
          </section>
          <section className='col-6'>
        ...
        <div className="row gx-3 gy-3 " id={styles.me}>
          {ps3Data.map((game, index) => (
            <section className="col-xxl-2 col-xl-3 col-md-6 col-4 mt-0" data-aos="fade-down"
              ...
              <h5 className="text-white mt-2 mb-0">Kr {game.price},-</h5>
              <hr />
            </section>
          ))}
        </div>
        ...
        <div className='row justify-content-center'>
          <button className='col-lg-2 btn btn-dark d-block '  id={styles.lessMore} onClick={loadMoreGames}>
            <i ></i> {buttonText}
          </button>
        </div>
      </div>
