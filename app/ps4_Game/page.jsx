...
import { getData } from '../ps4_Game/data';
...
const Page = async ({searchParams}) => {
  ...
  const ps4 = await getData(id + `?timestamp=${Date.now()}`);
    ...
    <div className="container" id={styles.cus_container}>
      ...
      <div className='row mt-5 ' id={styles.bg_row}>
        ...
        <section className='col-12 mx-auto col-xxl-6 p-5 pb-0'>
            ...
            <Image
              src={ps4.data[0].largeimg_url}
              alt={ps4.data[0].title}
              width={0}
              priority
              height={0}
              sizes="100vw"
        ...
        <section id={styles.bg_product} className='col-12 col-xxl-12 p-5 pt-2 pb-0'>
          <h1 className='p-0'>{ps4.data[0].title}</h1>
          <small className='col-12 p-0'>Varenummer: {ps4.data[0].prodnumber}</small>
          <p className='m-0 col-12 p-0'>Release Date: {ps4.data[0].release_date}</p>
          <p className='m-0 col-12 p-0 text-white'>Genre: {ps4.data[0].genre}</p>

          <hr />
        </section>
        <section className='col-12 col-xxl-4 p-5 pt-0'>
          <h2 className='col-12 p-0 col-xxl-12 '>Kr {ps4.data[0].price},-</h2>
        ...
        <p
              className="p-3 mb-0"
              id={styles.smallDes}
              dangerouslySetInnerHTML={{ __html: formatDescription(ps4.data[0].description) }}
            ></p>
        </section>
      </div>
    </div>
